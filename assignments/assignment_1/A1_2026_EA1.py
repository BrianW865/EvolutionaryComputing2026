...
# --- Parent Selection --- #
# Adapted from the 1_body_evolution_tree example.
# Chosen over tournament selection for stronger, more consistent selection pressure
def parent_selection(population: Population) -> Population:
    """Select top 50% as parents."""
    population = population.sort(sort="min")
    cutoff = len(population) // 2
    for i, ind in enumerate(population):
        ind.tags["selected"] = i < cutoff
